-- Write a query to return the 10 earliest orders in the orders 
...
-- according to oldest to newest, but with the smallest 
-- total_amt_usd for each date listed first for each date. 
...
-- have a dollar amount of gloss_amt_usd greater than or equal 
-- to 1000.
...
-- each order. Limit the results to the first 10 orders, 
-- and include the id and account_id fields. 

SELECT id,account_id,
	(standard_amt_usd / standard_qty ) as standard_amt_per_qty
FROM orders
LIMIT 10;

-- Write a query that finds the percentage of revenue that 
-- comes from poster paper for each order. You will need to
-- use only the columns that end with _usd. (Try to do this 
-- without using the total column). Include the id and 
-- account_id fields. 

SELECT id,account_id,
	((poster_amt_usd / total_amt_usd )*100) as standard_amt_per_qty
FROM orders;
...
-- where the standard_qty is over 1000, the 
...
-- Using the accounts table find all the 
-- companies whose names do not start with 'C' 
-- and end with 's'.
...
-- information regarding individuals who were 
-- contacted via organic or adwords and started 
-- their account at any point in 2016 sorted from 
-- newest to oldest.
...
-- poster_qty is greater than 4000. Only include the id 
